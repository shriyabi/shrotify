    ...
    <div className="flex items-center gap-3 bg-gray-800/50 p-2 rounded-lg hover:bg-gray-700 transition-colors group">
      ...
      <div className="flex-1 min-w-0">
        ...
        <div className="flex items-center gap-2 mt-0.5">
          ...
          {genreName && <span className="bg-indigo-900/40 border border-indigo-500/30 text-indigo-300 px-1.5 py-0.5 rounded text-[10px] uppercase tracking-wider whitespace-nowrap">{genreName}</span>}
        </div>
      </div>
      {meta?.previewUrl && (
...
function PlaylistCard({ playlistTitle, tracks, activeUrl, setActiveUrl }) {
  return (
    <div className="bg-gray-900/80 backdrop-blur-sm border border-gray-800 rounded-2xl p-5 shadow-lg">
      <div className="flex items-center justify-between mb-4 border-b border-gray-800 pb-2">
        <h3 className="text-xl font-bold text-indigo-400">{playlistTitle}</h3>
        <span className="text-xs font-mono text-gray-500 bg-gray-950 px-2 py-1 rounded-full">{tracks?.length || 0} tracks</span>
      </div>
...
export default function Playlists({ playlists, playlistMeta }) {
  ...
  const toggleFavorite = (playlist) => {
    ...
    if (favorites.includes(playlist)) {
      ...
      // Automatically remove from explore if adding to favorites
  ...
  const toggleExplore = (playlist) => {
    ...
    if (explore.includes(playlist)) {
      ...
      // Automatically remove from favorites if adding to explore
  ...
  // Ensure we don't try to render playlists that were deleted by admin but still in local storage
  ...
  return (
    <div className="space-y-12 pb-12">
      ...
      <div className="flex items-center justify-between border-b border-gray-800 pb-4 relative z-50" ref={dropdownRef}>
        ...
        <div className="relative">
          <button 
            ...
            className="flex items-center gap-2 bg-indigo-600/10 text-indigo-400 border border-indigo-500/30 hover:bg-indigo-600/20 px-4 py-2 rounded-lg text-sm font-bold transition-all"
          >
                ...
                {allPlaylistsList.map(playlist => {
                  ...
                  return (
                    <div 
                      ...
                      className="flex items-center justify-between p-2 hover:bg-gray-800 rounded-lg group transition-colors"
                    >
      ...
      <div>
        <div className="flex items-center gap-2 mb-6">
          ...
          <h2 className="text-xl font-bold text-gray-300">Favorites</h2>
        </div>
        ...
        {validFavorites.length > 0 ? (
          ...
          <div className="bg-gray-900/40 border border-gray-800 border-dashed rounded-2xl p-6 text-center text-gray-500 text-sm">
      ...
      <div>
        <div className="flex items-center gap-2 mb-6 mt-12">
          ...
          <h2 className="text-xl font-bold text-gray-300">Explore</h2>
        </div>
