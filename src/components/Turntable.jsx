...
export default function Turntable({ songData }) {
  ...
  useEffect(() => {
    ...
    fetch(`https://itunes.apple.com/search?term=${encodeURIComponent(songData.title)}&entity=song&limit=1`)
      .then(res => res.json())
  ...
  return (
    <section className="bg-gray-900 p-8 rounded-3xl border border-gray-800 flex flex-col items-center shadow-2xl">
      <h2 className="text-xl font-bold mb-8 text-gray-400 tracking-widest uppercase text-xs">Now Spinning: Song of the Day</h2>
      ...
      <div className="text-center mb-8">
        <h3 className="text-2xl font-bold text-white mb-1">{songData.title.split('-')[0]}</h3>
        <p className="text-indigo-400 font-mono text-sm">{songData.title.split('-')[1] || 'Unknown Artist'}</p>
      </div>
