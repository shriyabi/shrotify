    ...
    <section className="group relative overflow-hidden rounded-[30px] border border-white/15 bg-white/5 p-4 shadow-[0_25px_80px_rgba(15,23,42,0.85)] backdrop-blur-xl backdrop-saturate-150 sm:p-5 md:p-6 lg:p-7">
      ...
      <div className="relative z-10">
        ...
        <div className="relative flex items-center justify-center py-3">
          ...
          <div className={`relative flex h-32 w-32 items-center justify-center rounded-full border-[8px] border-slate-800/80 bg-[radial-gradient(circle,_#0f172a_0%,_#020617_58%,_#000_100%)] shadow-[0_0_40px_rgba(15,23,42,0.9)] sm:h-36 sm:w-36 md:h-40 md:w-40 lg:h-48 lg:w-48 aspect-square ${isPlayingPreview ? 'animate-spin-slow' : ''}`}>
            ...
            <div className="relative z-10 flex h-12 w-12 items-center justify-center overflow-hidden rounded-full border-4 border-slate-900 bg-slate-800 shadow-[0_0_25px_rgba(15,23,42,0.8)] sm:h-16 sm:w-16 md:h-20 md:w-20 aspect-square">
              <img
              ...
              <div className="absolute h-3 w-3 rounded-full border border-slate-700 bg-slate-950 shadow-[0_0_0_1px_rgba(255,255,255,0.1)]" />
            </div>
          </div>
        </div>

        <div className="mt-6 text-center">
          <h3 className="text-xl font-black tracking-tight text-white sm:text-2xl">{songData.song}</h3>
          <p className="mt-1 font-mono text-xs text-indigo-200 sm:text-sm">{songData.artist}</p>
        </div>

        <button
          onClick={togglePreview}
          disabled={!meta?.previewUrl}
          className="mt-6 flex w-full items-center justify-center gap-2 rounded-2xl bg-gradient-to-r from-indigo-500/90 via-violet-500/90 to-fuchsia-500/90 px-4 py-3 text-sm font-bold text-white shadow-[0_0_25px_rgba(99,102,241,0.32)] backdrop-blur-md transition-all hover:scale-[1.01] hover:shadow-[0_0_30px_rgba(168,85,247,0.4)] disabled:cursor-not-allowed disabled:opacity-50 sm:text-base"
        >
          {isPlayingPreview ? <Pause className="h-5 w-5" /> : <Play className="h-5 w-5" />} 30s Sample
        </button>
      </div>
    </section>
  );
